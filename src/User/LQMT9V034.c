...
{     
  ...
  if((PORTD_ISFR & 0x8000))//(1<<15)
  {
    ...
    // 用户程序 
    Line_Cont = 0;         //行计数清零
    V034_Field_Over_Flag=1;     //场结束标识
    
    GPIO_Reverse (PTA, 25);         //测试帧率，66帧/秒
  } 
}
...
//测试主函数
void Test_LQV034(u16 Pixle1[LCDH][LCDW])
{  
  int i,j;
 LCD_Show_Frame94();      //画图像 LCDW*LCDH 外框
  LQMT9V034_Init();        //摄像头初始化
    ...
    if(V034_Field_Over_Flag)    //完成一场图像采集后显示并发送数据到上位机
    {
      EXTI_Init(PTD,14,rising_down); //行中断关闭
      EXTI_Init(PTD,15,rising_down); //场中断关闭  
...
void yindaoxian(void)
{
  ...
  {  	
    ...
    for(nc=MAX_COL/2-2;nc<MAX_COL/2+2;nc++)
     Pixle[nr][nc+temp]=0;
  }
