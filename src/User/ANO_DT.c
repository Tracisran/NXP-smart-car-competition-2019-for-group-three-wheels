...
void ANO_DT_Send_Data(s8 *dataToSend , u8 length)
{
	
	UART_Put_Buff(UART_4, dataToSend, length);     //可以修改不同的串口发送数据
	
}
s8 data_to_send[50];	//发送数据缓存
...
/*发送给上位机的数据协议*/
void ANO_DT_send_int16(short data1, short data2, short data3, short data4, short data5/*,short data7, short data8, short .....可根据需要自行添加 */)
{
    uint8_t _cnt=0;
    data_to_send[_cnt++] = 0xAA;      //匿名协议帧头  0xAAAA
	data_to_send[_cnt++] = 0xAA;
	data_to_send[_cnt++] = 0x02;      //使用用户协议帧0xF1  
    data_to_send[_cnt++] = 0;        //8个short 长度 16个字节
